...
Cathmhaol.ProfileProgress = function(img, comp, canvas, imgHeight, height, width, thickness, color, bgColor, overlay, font, animate, bar, sqr) {
	 ...
	 */
	function drawBar(pc) {
		var objGradientBar = CONTEXT.createLinearGradient(PROGRESS.x, PROGRESS.y, PROGRESS.x, Math.floor(PROGRESS.height * 1.5))
		  , borderWidth = 3
		;
		...
		CONTEXT.fillRect(PROGRESS.x - borderWidth, PROGRESS.y - (borderWidth * 1.5), PROGRESS.width + (borderWidth * 2), PROGRESS.height + borderWidth);

		/* Render the bar */
		objGradientBar.addColorStop(0, color.hex);
		objGradientBar.addColorStop(1, "#fff");
		CONTEXT.fillStyle = objGradientBar;
		CONTEXT.fillRect(PROGRESS.x - borderWidth, PROGRESS.y - borderWidth, Math.round(pc * PROGRESS.width), PROGRESS.height);

		/* Render the border */
		CONTEXT.strokeStyle = color.darker;
		CONTEXT.rect(PROGRESS.x - borderWidth, PROGRESS.y - borderWidth, Math.round(pc * PROGRESS.width), PROGRESS.height);
		CONTEXT.stroke();
	...
	function drawCircle(pc) {
		...
		CONTEXT.arc(CENTER.x, CENTER.y, RADIUS, startingAngle, startingAngle + ((Math.PI * 2) * pc));
		...
		CONTEXT.save();
		CONTEXT.beginPath();
		CONTEXT.arc(CENTER.x, CENTER.y, RADIUS - thickness, 0, Math.PI * 2);
		CONTEXT.closePath();
		CONTEXT.fillStyle = bgColor.hex;
		CONTEXT.fill();
		CONTEXT.restore();
	...
	function drawPic() {
		...
		if (PIC) {
			...
			CONTEXT.beginPath();
			if (!sqr) {
				CONTEXT.arc(CENTER.x, CENTER.y, (RADIUS - thickness) + 10, 0, Math.PI * 2, true);
			} else {
	...
	function drawText(pc) {
		if (overlay) {
			...
			CONTEXT.fillStyle = "rgba(" + color.red + ", " + color.green + ", " + color.blue + ", 0.7)";
			CONTEXT.fillText(overlay.replace(/\b\d{1,}\b/, Math.floor(pc * 100)), CENTER.x, CENTER.y);
		}
		return;
	}
	...
	function translateColor(color) {
		...
		if (color) {
			obj.blue = color.blue ? color.blue : short ? parseInt(short[3] + short[3], 16) : full ? parseInt(full[3], 16) : 0;
			obj.green = color.green ? color.green : short ? parseInt(short[2] + short[2], 16) : full ? parseInt(full[2], 16) : 0;
			obj.red = color.red ? color.red : short ? parseInt(short[1] + short[1], 16) : full ? parseInt(full[1], 16) : 0;
			...
			obj.hex = "#" + ("0" + Math.floor(obj.red).toString(16)).substr(-2) + ("0" + Math.floor(obj.green).toString(16)).substr(-2) + ("0" + Math.floor(obj.blue).toString(16)).substr(-2);
			...
			obj.darker = "#" + ("00000" + (g | (b << 8) | (r << 16)).toString(16)).substr(-6);
		}
	...
	var APSECT                         /* Image height/width ratio                     */
	  , CONTAINER                      /* Element that will contain the canvas element */
	 ...
	 */
	height = Math.max(height || DEF.rad, PIC.height || DEF.rad, canvas.height || DEF.rad);
	width = Math.max(width || DEF.rad, PIC.width || DEF.rad, canvas.width || DEF.rad);
	if (height === canvas.height || width === canvas.width || height === PIC.height || width === PIC.width) {
