...
-----
### *Financial*
This library provides a variety of financial calulators.

###### Constructor Parameters
- None

###### Example
var schedule = Cathmhaol.Financial.amortize(100000.00, 6.5, 30, true);
...
##### Methods
- ***object[]* amortize(*float* principal, *float* apr, *integer* years, *boolean* balloon)**: Returns an object array containing payment details for each periodic payment on an amortized amount.
    * balance: balance remaining after the payment
...
- ***float* assetTurnover(*float* sales, *float* assets)**: Returns a measure of how efficiently a company's assets generate revenue in the form of units of revenue per unit of asset (e.g., number of dollars generated by each dollar in assets).
- ***float* breakEvenPoint(*float* fixedCost, *float* sellingPrice, *float* variableCost)**: Returns the number of unit sales required to cover costs.
- ***float* capitalLiquidity(*float* capital, *float* assets)**: Returns a measure of current liquidity.
- ***float* compoundInterest(*float* principal, *float* apr, *integer* periods)**: Returns an amount of interest compounded for the specified number of periods.
- ***float* debtEquity(*float* liabilities, *float* equity)**: Returns a measure of a company's leverage, indicating what proportion of equity and debt the company is using to finanace its assets.
- ***float* equity(*float* assets, *float* liabilities)**: Returns a measure of assets that are financed by stockholders.
- ***float* futureValue(*float* payment, *float* apr, *integer* periods)**: Returns the future value of periodic payments (e.g., an annuity).
- ***float* profitMargin(*float* profit, *float* sales)**: Returns the percentage of a sale that is profit.
- ***float* retainedEarnings(*float* retainedEarnings, *float* assets)**: Returns the ratio of retained earnings to assets, e.g., a ratio greater than or equal to 1 indicates growth is sustainable (growth financed by profits).
- ***float* returnOnAssets(*float* earnings, *float* assets)**: Returns a measure of earnings to assets, giving an indicator of efficiency in using assets to generate revenue.
- ***float* returnOnEquity(*float* profit, *float* equity)**: Returns the amount of net income as a percentage of shareholder equity, revealing how much profit a company generates with the money shareholders have invested.
- ***string* ruleOfSeventyTwo(*float* amount, *boolean* bAPR)**: Returns either the amount of time required or the interest rate required for an investment to double.
- ***float* zScore(*float* earnings, *float* sales, *float* equity, *float* capital, *float* retained, *float* assets, *float* liabilities)**: Returns the Altman Z-Score using the earnings and sales for a period, the owner's equity, working capital, retained earnings, and the assets and liabilities held by the organization. According to the theory, by calculating how liquid a company is and how strong their cash flow, we can determine whether or not the company is likely to fail. A score less than 1.8 indicates a high probability of failure; a score between 1.8 and 2.6 indicates a moderate/high probability of failure; a score between 2.7 and 2.9 indicates a  moderate probability of failure; and a score greater than 2.9 indicates a low probability of failure.
